...
By picking a common strucutre for our data, we are able to transform, plot and analyse it using a common set of functions. We don't need to move from one method to another depending our what our data looks like. 
...
- Factors, a stricter type for characters to use with catagorical variables.
...
These three packages are _installed_ part of the `tidyverse` and are installed with it. `stringr` and `forcats` are _loaded_ with `library(tidyverse)`, but `lubridate` isn't and needs to be loaded seprately.
...
This tells us that our data **structure** is `tbl_df`, `tbl` and `data.frame` (ie a `tibble`), and the data **types** of our variables are: character, numeric, character, character and character.
...
If we want to change that, we redefine the `numb2` variable using the `mutate` function (from the previous chapter) and the `as.numeric` function descibed in the previous section:
...
We do convert the `numb3` variable to a numeric, which is what we wanted. But there are casualites. Because `"None"` cannot be neatly converted to a number, the string in the last observation is removed and replaced with `NA` (and we get a warning that there were `NAs introduced by coercion`). 
